...
public class IssueOfIssueList {
    ...
    public IssueOfIssueList(Issue issue) {
        ...
        this.assignees = issue.getAssignees().stream().map(IssueAssignee::getAssignee).map(UserOfIssue::new).collect(toList());
        this.labels = issue.getLabels().stream().map(IssueLabel::getLabel).map(LabelOfIssue::new).collect(toList());
